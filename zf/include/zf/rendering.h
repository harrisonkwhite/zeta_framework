...
namespace zf {
    ...
    constexpr t_s32 g_batch_vert_component_cnt = sizeof(s_batch_vert) / sizeof(t_f32);

    static_assert([]() {
        t_s32 sum = 0;

        for (t_s32 i = 0; i < g_batch_vert_attr_lens.Len(); i++) {
    ...
    constexpr t_s32 g_batch_slot_cnt = 1 << 8;
    ...
    struct s_rendering_basis {
        ...
        [[nodiscard]] t_b8 Init(c_gfx_resource_arena& gfx_res_arena, c_mem_arena& temp_mem_arena);
    };
    ...
    class c_renderer {
    public:
        t_b8 Init(c_gfx_resource_arena& gfx_res_arena, c_mem_arena& mem_arena, c_mem_arena& temp_mem_arena);
        ...
        void Clear(const c_color_rgba_32f col = {}) const;
        void SetViewMatrix(const s_matrix_4x4& mat);
        void DrawTexture(const s_texture& tex, const s_v2<t_f32> pos, const s_rect<t_s32> src_rect = {}, const s_v2<t_f32> origin = origins::g_topleft, const s_v2<t_f32> scale = {1.0f, 1.0f}, const t_f32 rot = 0.0f, const c_color_rgba_32f blend = colors::g_white);

        void DrawRect(const s_rect<t_f32> rect, const c_color_rgba_32f color) {
            DrawTexture(m_basis.px_tex, rect.Pos(), {}, {}, rect.Size(), 0.0f, color);
        }

        void DrawLine(const s_v2<t_f32> a, const s_v2<t_f32> b, const c_color_rgba_32f blend, const t_f32 width) {
    ...
    private:
        ...
        t_s32 m_batch_slots_used_cnt = 0;

        s_matrix_4x4 m_batch_view_mat;

        s_gfx_resource_handle m_batch_tex_hdl;

        void Draw(const s_gfx_resource_handle tex_hdl, const s_rect<t_f32> tex_coords, s_v2<t_f32> pos, s_v2<t_f32> size, s_v2<t_f32> origin, const t_f32 rot, const c_color_rgba_32f blend);
        void Flush();
    };
}
