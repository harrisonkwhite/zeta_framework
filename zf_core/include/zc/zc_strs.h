...
namespace zf {
    ...
    t_b8 WalkStr(const s_str_rdonly str, t_size& byte_index, t_unicode_code_pt& o_code_pt); // Returns false iff the walk has ended.
    t_b8 WalkStrReverse(const s_str_rdonly str, t_size& byte_index, t_unicode_code_pt& o_code_pt); // Returns false iff the walk has ended. For a full walk, the byte index has to be initialised to the index of ANY BYTE in the last character.
    ...
    constexpr t_b8 IsStrTerminatedAnywhere(const s_str_rdonly str) {
        // The terminator is most likely at the end, so we start there.
        for (t_size i = str.bytes.len - 1; i >= 0; i--) {
            if (!str.bytes[i]) {
                return true;
    ...
    t_b8 IsValidUTF8Str(const s_str_rdonly str);
    t_size CalcStrLen(const s_str_rdonly str);
    [[nodiscard]] t_b8 CalcStrLenAndCheckValidity(const s_str_rdonly str, t_size& o_len); // Returns false iff the provided string is not valid UTF-8 form.
    t_b8 WalkStr(const s_str_rdonly str, t_size& byte_index, t_unicode_code_pt& o_code_pt); // Returns false iff the walk has ended.
    t_b8 WalkStrReverse(const s_str_rdonly str, t_size& byte_index, t_unicode_code_pt& o_code_pt); // Returns false iff the walk has ended. For a full walk, byte index has to be initialised to the index of ANY BYTE in the last character.
    t_unicode_code_pt UTF8ChrBytesToCodePoint(const s_array_rdonly<t_u8> bytes); // Only accepts bytes representing a unicode code point in an array with a length in [1, 4].
    void MarkStrCodePoints(const s_str_rdonly str, t_unicode_code_pt_bit_vector& code_pt_bv); // Sets the bits associated with each unicode code point that appear in the input string.
#endif
