...
namespace zf {
#if 0
    int IndexOfFirstSetBit(const t_u8 xor = 0) {
        ...
        for (int i = 0; i < m_bytes.Len(); i++) {
            const int bi = fs_mappings[m_bytes[i] ^ xor];
    ...
    static t_u8 LeftShiftOnByte(t_u8& byte, const size_t amount, const t_u8 carry = 0, const t_u8 m_bit_cnt) {
        assert(amount <= 8);
        ...
        byte <<= amount;
        byte |= carry;
        return (byte_old >> (m_bit_cnt - amount)) & ((1 << amount) - 1);
    }

    static t_u8 RightShiftOnByte(t_u8& byte, const size_t amount, const t_u8 carry = 0, const t_u8 m_bit_cnt) {
        assert(amount <= 8);
        assert(carry < (1 << amount));
        assert(m_bit_cnt <= 8);
        const t_u8 byte_old = byte;
        byte <<= amount;
        byte |= carry >> (8 - m_bit_cnt);
        return (byte_old & ((1 << amount) - 1)) << ;
    }

    void And(const c_array<t_u8> m_bytes, const c_array<const t_u8> mask) {
        assert(m_bytes.Len() == mask.Len());
    ...
    void Or(const c_array<t_u8> m_bytes, const c_array<const t_u8> mask) {
        assert(m_bytes.Len() == mask.Len());
    ...
    void Xor(const c_array<t_u8> m_bytes, const c_array<const t_u8> mask) {
        assert(m_bytes.Len() == mask.Len());
