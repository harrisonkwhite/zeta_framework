...
namespace zf {
    static t_b8 OutputCode(const s_str_rdonly input_file_path, const s_str_rdonly output_file_path, const s_str_rdonly arr_subname) {
        ...
        Print(&output_file_stream, c_cstr_literal{"#include <zcl/zcl_mem.h>\n"});
        Print(&output_file_stream, c_cstr_literal{"\n"});
        Print(&output_file_stream, c_cstr_literal{"namespace zf {\n"});

        PrintFormat(&output_file_stream, c_cstr_literal{"    extern const t_u8 g_%_raw[] = {"}, arr_subname);
        ...
        while (input_file_stream.ReadItem(&byte_read)) {
            ...
            PrintFormat(&output_file_stream, c_cstr_literal{"%"}, FormatHex(byte_read));
        ...
        PrintFormat(&output_file_stream, c_cstr_literal{"    extern const t_i32 g_%_len = %;\n"}, arr_subname, byte_read_cnt);
...
int main(const int arg_cnt, const char *const *const args) {
    if (arg_cnt != 4) {
        zf::LogError(zf::c_cstr_literal{"Invalid command-line argument count!"});
        return EXIT_FAILURE;
    }
