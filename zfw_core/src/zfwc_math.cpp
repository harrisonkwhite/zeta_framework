...
s_poly GenQuadPoly(s_mem_arena& mem_arena, const s_v2 pos, const s_v2 size, const s_v2 origin) {
    ...
    *V2Elem(pts, 3) = s_v2{pos_base.x, pos_base.y + size.y};
    ...
    return s_poly{.pts = V2ArrayView(pts)};
}

s_poly GenQuadPolyRotated(s_mem_arena& mem_arena, const s_v2 pos, const s_v2 size, const s_v2 origin, const t_r32 rot) {
    assert(origin.x >= 0.0f && origin.x <= 1.0f && origin.y >= 0.0f && origin.y <= 1.0f);
...
bool DoesPolyIntersWithRect(const s_poly poly, const s_rect rect) {
    ...
    const s_poly rect_poly{.pts = ARRAY_FROM_STATIC(pts)};
