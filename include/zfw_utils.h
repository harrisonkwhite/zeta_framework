...
inline void ActivateBit(const int bit_index, t_byte* const bytes, const int bit_cnt) {
...
inline void DeactivateBit(const int bit_index, t_byte* const bytes, const int bit_cnt) {
    assert(bit_index >= 0 && bit_index < bit_cnt);
    assert(bytes);
    assert(bit_cnt > 0);
