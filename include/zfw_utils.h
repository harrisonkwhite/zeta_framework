...
static inline void ZFWZeroOut(void* const mem, const size_t size) {
    assert(mem);
    assert(size > 0);
...
static inline bool ZFWIsPowerOfTwo(const size_t n) {
    return n > 0 && (n & (n - 1)) == 0;
}

static inline bool ZFWIsValidAlignment(const size_t n) {
    return n > 0 && ZFWIsPowerOfTwo(n);
}

static inline size_t ZFWAlignForward(const size_t n, const size_t alignment) {
...
static inline bool ZFWIsMemArenaValid(zfw_s_mem_arena* arena) {
...
static inline void ZFWActivateBit(const size_t bit_index, zfw_t_byte* const bytes, const size_t bit_cnt) {
...
static inline void ZFWDeactivateBit(const size_t bit_index, zfw_t_byte* const bytes, const size_t bit_cnt) {
    assert(bit_index < bit_cnt);
    assert(bytes);
    assert(bit_cnt > 0);
...
static inline bool ZFWIsBitActive(const size_t bit_index, const zfw_t_byte* const bytes, const size_t bit_cnt) {
    assert(bit_index < bit_cnt);
    assert(bytes);
    assert(bit_cnt > 0);
...
static inline zfw_t_byte ZFWKeepFirstNBitsOfByte(const zfw_t_byte byte, const size_t n) {
    assert(n <= 8);
    return byte & ((1 << n) - 1);
}
