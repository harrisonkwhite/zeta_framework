...
namespace zf::input {
    ...
    t_b8 key_is_down(const t_state *const state, const t_key_code code) {
        return mem::is_bit_set(state->keys_down, code);
    }

    t_b8 key_is_pressed(const t_state *const state, const t_key_code code) {
        return mem::is_bit_set(state->events.keys_pressed, code);
    }

    t_b8 key_is_released(const t_state *const state, const t_key_code code) {
        return mem::is_bit_set(state->events.keys_released, code);
    }

    void key_update_state(t_state *const state, const t_key_code code, const t_b8 is_down) {
        if (is_down) {
            if (!mem::is_bit_set(state->keys_down, code)) {
                mem::set_bit(state->keys_down, code);
                mem::set_bit(state->events.keys_pressed, code);
            }
        } else {
            if (mem::is_bit_set(state->keys_down, code)) {
                mem::unset_bit(state->keys_down, code);
                mem::set_bit(state->events.keys_released, code);
            }
        }
    }

    t_b8 mouse_button_is_down(const t_state *const state, const t_mouse_button_code btn_code) {
        return mem::is_bit_set(state->mouse_buttons_down, static_cast<t_i32>(btn_code));
    }

    t_b8 mouse_button_is_pressed(const t_state *const state, const t_mouse_button_code btn_code) {
        return mem::is_bit_set(state->events.mouse_buttons_pressed, static_cast<t_i32>(btn_code));
    }

    t_b8 mouse_button_is_released(const t_state *const state, const t_mouse_button_code btn_code) {
        return mem::is_bit_set(state->events.mouse_buttons_released, static_cast<t_i32>(btn_code));
    }

    void mouse_button_update_state(t_state *const state, const t_mouse_button_code btn_code, const t_b8 is_down) {
        if (is_down) {
            if (!mem::is_bit_set(state->mouse_buttons_down, static_cast<t_i32>(btn_code))) {
                mem::set_bit(state->mouse_buttons_down, static_cast<t_i32>(btn_code));
                mem::set_bit(state->events.mouse_buttons_pressed, static_cast<t_i32>(btn_code));
            }
        } else {
            if (mem::is_bit_set(state->mouse_buttons_down, static_cast<t_i32>(btn_code))) {
                mem::unset_bit(state->mouse_buttons_down, static_cast<t_i32>(btn_code));
                mem::set_bit(state->events.mouse_buttons_released, static_cast<t_i32>(btn_code));
            }
        }
    }
    ...
    t_b8 gamepad_is_connected(const t_state *const state, const t_i32 index) {
        ZF_ASSERT(index >= 0 && index < g_gamepad_limit);
        return mem::is_bit_set(state->gamepads_connected, index);
    }

    t_b8 gamepad_is_button_down(const t_state *const state, const t_i32 gamepad_index, const t_gamepad_button_code btn_code) {
        ZF_ASSERT(gamepad_is_connected(state, gamepad_index));
        return mem::is_bit_set(state->gamepads[gamepad_index].buttons_down, btn_code);
    }

    t_b8 gamepad_is_button_pressed(const t_state *const state, const t_i32 gamepad_index, const t_gamepad_button_code btn_code) {
        ZF_ASSERT(gamepad_is_connected(state, gamepad_index));
        return mem::is_bit_set(state->events.gamepads[gamepad_index].buttons_pressed, btn_code);
    }

    t_b8 gamepad_is_button_released(const t_state *const state, const t_i32 gamepad_index, const t_gamepad_button_code btn_code) {
        ZF_ASSERT(gamepad_is_connected(state, gamepad_index));
        return mem::is_bit_set(state->events.gamepads[gamepad_index].buttons_released, btn_code);
    }
    ...
    void gamepad_update_state(t_state *const state, const t_i32 gamepad_index, const t_b8 connected, const mem::t_static_bitset<ecm_gamepad_button_code_cnt> &btns_down, const t_static_array<t_f32, ecm_gamepad_axis_code_cnt> &axes) {
        ...
        if (!mem::is_bit_set(state->gamepads_connected, gamepad_index)) {
            mem::set_bit(state->gamepads_connected, gamepad_index);
            state->gamepads[gamepad_index] = {};
            state->events.gamepads[gamepad_index] = {};
        }

        for (t_i32 i = 0; i < ecm_gamepad_button_code_cnt; i++) {
            if (mem::is_bit_set(btns_down, i)) {
                if (!mem::is_bit_set(state->gamepads[i].buttons_down, i)) {
                    mem::set_bit(state->gamepads[i].buttons_down, i);
                    mem::set_bit(state->events.gamepads[i].buttons_pressed, i);
                }
            } else {
                if (mem::is_bit_set(state->gamepads[i].buttons_down, i)) {
                    mem::unset_bit(state->gamepads[i].buttons_down, i);
                    mem::set_bit(state->events.gamepads[i].buttons_released, i);
                }
            }
        }
