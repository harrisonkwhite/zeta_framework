...
namespace zf {
    ...
    t_b8 CreateAudioSys(const s_ptr_nonnull<s_mem_arena> mem_arena, const s_ptr_nonnull<s_ptr<s_audio_sys>> o_as) {
    ...
    t_b8 CreateSoundTypeFromRaw(const s_str_rdonly file_path, const s_ptr_nonnull<s_sound_type_arena> type_arena, const s_ptr_nonnull<s_mem_arena> temp_mem_arena, const s_ptr_nonnull<s_ptr<s_sound_type>> o_type) {
    ...
    void DestroySoundTypes(const s_ptr_nonnull<s_sound_type_arena> type_arena) {
        const s_ptr_nonnull<s_audio_sys> as = type_arena->audio_sys;
    ...
    t_b8 PlaySound(const s_ptr_nonnull<const s_sound_type> type, const s_ptr_nonnull<s_sound_id> o_id, const t_f32 vol, const t_f32 pan, const t_f32 pitch, const t_b8 loop) {
        ...
        const s_ptr_nonnull<s_audio_sys> as = type->audio_sys;
        ...
        const s_ptr_nonnull<ma_sound> ma_snd = &as->snd_insts.ma_snds[index];
        const s_ptr_nonnull<ma_audio_buffer_ref> ma_buf_ref = &as->snd_insts.ma_buf_refs[index];
    ...
    t_b8 IsSoundPlaying(const s_ptr_nonnull<s_audio_sys> as, const s_sound_id id) {
    ...
    void ProcFinishedSounds(const s_ptr_nonnull<s_audio_sys> as) {
        ZF_FOR_EACH_SET_BIT(as->snd_insts.activity, i) {
            const s_ptr_nonnull<ma_sound> ma_snd = &as->snd_insts.ma_snds[i];
