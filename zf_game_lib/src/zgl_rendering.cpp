...
namespace zgl {
    ...
    static zcl::t_array_mut<zcl::t_v2> CalcStrLineChrOffsets(const zcl::t_str_rdonly str, const zcl::t_font_arrangement &font_arrangement, zcl::t_arena *const arena) {
        ...
        zcl::t_v2 offs_top_left = {zcl::k_f32_inf_pos, zcl::k_f32_inf_pos};
        ...
        ZCL_STR_WALK (str, step) {
            ...
            offs_top_left.x = zcl::CalcMin(chr_offs_pen.x + static_cast<zcl::t_f32>(glyph_info->offs.x), offs_top_left.x);
            offs_top_left.y = zcl::CalcMin(chr_offs_pen.y + static_cast<zcl::t_f32>(glyph_info->offs.y), offs_top_left.y);
        ...
        ZCL_STR_WALK (str, step) {
            result[chr_index] -= offs_top_left;
            chr_index++;
        }
    ...
    zcl::t_array_mut<zcl::t_poly_mut> CalcStrLineChrRenderColliders(const zcl::t_str_rdonly str, const t_font &font, const zcl::t_v2 pos, zcl::t_arena *const arena, zcl::t_arena *const temp_arena, const zcl::t_v2 origin, const zcl::t_f32 rot, const zcl::t_v2 scale) {
        ...
        const t_str_chr_render_info_rdonly chr_render_info = CalcStrLineRenderInfo(str, font.arrangement, origin, temp_arena);

        const auto result = zcl::ArenaPushArray<zcl::t_poly_mut>(arena, chr_render_info.chr_offsets.len);
        ...
        ZCL_STR_WALK (str, step) {
            ...
            const zcl::t_v2 chr_pos = pos + chr_render_info.offs_min + zcl::CalcLengthDir(chr_render_info.chr_offsets[chr_index].x * scale.x, rot) + zcl::CalcLengthDir(chr_render_info.chr_offsets[chr_index].y * scale.y, rot + (zcl::k_pi / 2.0f));
    ...
    void RendererSubmitStrLine(const t_rendering_context rc, const zcl::t_str_rdonly str, const t_font &font, const zcl::t_v2 pos, const zcl::t_color_rgba32f color, zcl::t_arena *const temp_arena, const zcl::t_v2 origin, const zcl::t_f32 rot, const zcl::t_v2 scale) {
        ...
        const zcl::t_array_rdonly<zcl::t_v2> chr_offsets = CalcStrLineChrOffsets(str, font.arrangement, temp_arena);
        ...
        RendererSubmitRect(rc, {pos.x - 4.0f, pos.y - 4.0f, 8.0f, 8.0f}, zcl::k_color_red);
