...
namespace zf
{
    ...
    // Returns false iff the load failed. Failure DOES NOT leave the underlying resource system in
    // an invalid state - you are safe to continue.
    ...
    // Returns false iff the load failed. Failure DOES NOT leave the underlying resource system in
    // an invalid state - you are safe to continue.
    [[nodiscard]] t_b8 LoadFontFromRaw(const s_str_rdonly file_path,
        const t_s32 height,
    ...
    // Returns false on failure. Failure DOES NOT leave the underlying resource system in an invalid
    // state - you are safe to continue.
    [[nodiscard]] t_b8 MakeSurface(
        const s_v2<t_s32> size, s_gfx_resource_arena& res_arena, s_gfx_resource*& o_surf);

    // Returns true iff the operation was successful. If it failed, the old surface state with its
    // old size is left intact.
    ...
    void DrawTexture(const s_rendering_context rc,
        ...
        const s_v2<t_f32> origin = origins::g_topleft,
        const s_v2<t_f32> scale = {1.0f, 1.0f},
        const t_f32 rot = 0.0f,
        const s_color_rgba32f blend = colors::g_white);
    void DrawRect(
    ...
    void DrawRectOpaqueOutlined(const s_rendering_context rc,
        ...
        const t_f32 outline_thickness = 1.0f);
    ...
    void DrawLine(const s_rendering_context rc,
        ...
        const t_f32 thickness = 1.0f);
    [[nodiscard]] t_b8 DrawStr(const s_rendering_context rc,
    ...
    struct s_surface_shader_prog_uniform_val
    {
        ...
        s_surface_shader_prog_uniform_val(const t_s32 v)
            : type(ek_surface_shader_prog_uniform_val_type_s32), type_data({.s32 = v}) {};
        s_surface_shader_prog_uniform_val(const t_u32 v)
            : type(ek_surface_shader_prog_uniform_val_type_u32), type_data({.u32 = v}) {};
        s_surface_shader_prog_uniform_val(const t_f32 v)
            : type(ek_surface_shader_prog_uniform_val_type_f32), type_data({.f32 = v}) {};
        s_surface_shader_prog_uniform_val(const s_v2<t_f32> v)
            : type(ek_surface_shader_prog_uniform_val_type_v2), type_data({.v2 = v}) {};
        s_surface_shader_prog_uniform_val(const s_v3<t_f32> v)
            : type(ek_surface_shader_prog_uniform_val_type_v3), type_data({.v3 = v}) {};
        s_surface_shader_prog_uniform_val(const s_v4<t_f32> v)
            : type(ek_surface_shader_prog_uniform_val_type_v4), type_data({.v4 = v}) {};
        s_surface_shader_prog_uniform_val(const s_matrix_4x4& v)
            : type(ek_surface_shader_prog_uniform_val_type_mat4x4), type_data({.mat4x4 = v}) {};
    };

    void SetSurfaceShaderProg(const s_rendering_context rc, const s_gfx_resource* const prog);
    [[nodiscard]] t_b8 SetSurfaceShaderProgUniform(const s_rendering_context rc,
