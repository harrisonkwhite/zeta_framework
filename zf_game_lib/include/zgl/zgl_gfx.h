...
namespace zf {
    ...
    [[nodiscard]] t_b8 ResizeSurface(s_gfx_resource *const surf, const s_v2i size);
    ...
    void DrawTexture(const s_rendering_context rc, const s_gfx_resource *const tex,
                     const s_v2 pos, const s_rect_i src_rect = {},
                     const s_v2 origin = origins::g_topleft, const s_v2 scale = {1.0f, 1.0f},
                     const t_f32 rot = 0.0f, const s_color_rgba32f blend = colors::g_white);
    ...
    void DrawLine(const s_rendering_context rc, const s_v2 a, const s_v2 b,
                  const s_color_rgba32f blend, const t_f32 thickness = 1.0f);
    [[nodiscard]] t_b8 DrawStr(const s_rendering_context rc, const s_str_rdonly str,
                               const s_gfx_resource *const font, const s_v2 pos,
                               const s_v2 alignment, const s_color_rgba32f blend,
                               s_mem_arena &temp_mem_arena);
    ...
    struct s_surface_shader_prog_uniform_val {
        ...
        s_surface_shader_prog_uniform_val(const s_v2 v)
            : type(ek_surface_shader_prog_uniform_val_type_v2), type_data({.v2 = v}) {};
        s_surface_shader_prog_uniform_val(const s_v3 v)
            : type(ek_surface_shader_prog_uniform_val_type_v3), type_data({.v3 = v}) {};
        s_surface_shader_prog_uniform_val(const s_v4 v)
            : type(ek_surface_shader_prog_uniform_val_type_v4), type_data({.v4 = v}) {};
        s_surface_shader_prog_uniform_val(const s_mat4x4 &v)
            : type(ek_surface_shader_prog_uniform_val_type_mat4x4),
              type_data({.mat4x4 = v}) {};
    };

    void SetSurfaceShaderProg(const s_rendering_context rc, const s_gfx_resource *const prog);
    [[nodiscard]] t_b8 SetSurfaceShaderProgUniform(
        const s_rendering_context rc, const s_str_rdonly name,
        const s_surface_shader_prog_uniform_val &val, s_mem_arena &temp_mem_arena);
