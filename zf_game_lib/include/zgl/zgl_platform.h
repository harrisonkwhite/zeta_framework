...
namespace zf {
    ...
    inline void ToggleFullscreen(const s_ptr_nonnull<s_platform_layer_info> pli) {
    ...
    namespace internal {
        [[nodiscard]] t_b8 InitPlatformLayer(const s_ptr_nonnull<s_mem_arena> mem_arena, const s_ptr_nonnull<s_input_state> input_state, const s_ptr_nonnull<s_ptr<s_platform_layer_info>> o_pli);
        void ShutdownPlatformLayer(const s_ptr_nonnull<const s_platform_layer_info> pli);

        void PollOSEvents();

        void ShowWindow(const s_ptr_nonnull<const s_platform_layer_info> pli);
        t_b8 ShouldWindowClose(const s_ptr_nonnull<const s_platform_layer_info> pli);
        void SwapWindowBuffers(const s_ptr_nonnull<const s_platform_layer_info> pli);
    }
}
