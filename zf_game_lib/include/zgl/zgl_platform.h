...
namespace zf {
    ...
    inline void ToggleFullscreen(s_platform_layer_info &pli) {
    ...
    namespace internal {
        [[nodiscard]] t_b8 InitPlatformLayer(s_mem_arena &mem_arena, s_input_state &input_state, s_ptr<s_platform_layer_info> &o_pli);
        void ShutdownPlatformLayer(const s_platform_layer_info &pli);

        void PollOSEvents();

        void ShowWindow(const s_platform_layer_info &pli);
        t_b8 ShouldWindowClose(const s_platform_layer_info &pli);
        void SwapWindowBuffers(const s_platform_layer_info &pli);
    }
}
