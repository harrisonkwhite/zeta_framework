...
namespace zf::renderer {
    ...
    inline s_resource_arena CreateGFXResourceArena(s_mem_arena &mem_arena) {
        return {.mem_arena = &mem_arena};
    }

    void DestroyGFXResources(s_resource_arena &arena);
    ...
    public:
        ...
        void SubmitMeshUpdate(const s_resource &mesh, const s_array_rdonly<t_f32> verts);
        void SubmitTextureSet(const s_resource &tex, const s_resource &sampler_uniform);
        void SubmitMeshDraw(const s_resource &mesh, const s_resource &prog, const t_i32 vert_cnt);
