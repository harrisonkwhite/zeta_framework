...
zcl::t_b8 compile_shader(const zcl::t_str_rdonly shader_file_path, const zcl::t_str_rdonly varying_def_file_path, const zcl::t_b8 is_frag, zcl::t_arena *const bin_arena, zcl::t_arena *const temp_arena, zcl::t_array_mut<zcl::t_u8> *const o_bin) {
    ...
    ZF_DEFER({
        if (r < 0) {
            ...
            zcl::io::log_error(ZCL_STR_LITERAL("%"), err);
        }
    });
    ...
    zcl::io::print_format(shaderc_file_path_terminated_byte_stream, ZCL_STR_LITERAL("%%\0"), exe_dir, shaderc_file_path_rel);
    ...
    zcl::io::print_format(shaderc_include_dir_terminated_byte_stream, ZCL_STR_LITERAL("%%\0"), exe_dir, shaderc_include_dir_rel);
