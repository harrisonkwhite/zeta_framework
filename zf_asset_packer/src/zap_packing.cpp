...
namespace zf
{
    ...
    static s_static_array<s_asset_field, eks_tex_field_cnt> g_tex_fields = {
        {{"src_file_path", ek_asset_field_type_str}, {"dest_file_path", ek_asset_field_type_str}}};
    ...
    static s_static_array<s_asset_field, eks_font_field_cnt> g_font_fields = {
        {{"src_file_path", ek_asset_field_type_str}, {"height", ek_asset_field_type_num},
    ...
    t_b8 RunPacker(const s_str_rdonly instrs_json_file_path)
    {
        ...
        const auto cj = [instrs_json_file_path, &mem_arena]() -> cJSON*
        {
            ...
            if (!LoadFileContents(
                    instrs_json_file_path, mem_arena, mem_arena, instrs_json_file_contents))
            {
                LogError(
                    "Failed to load packing instructions JSON file \"%\"!", instrs_json_file_path);
                return nullptr;
            }

            s_str instrs_json_str_terminated;

            if (!CloneStrButAddTerminator(
                    {instrs_json_file_contents}, mem_arena, instrs_json_str_terminated))
            {
                ZF_REPORT_ERROR();
                return nullptr;
            }
        ...
        for (t_size asset_type_index = 0; asset_type_index < eks_asset_type_cnt; asset_type_index++)
        {
            ...
            if (!CloneStrButAddTerminator(
                    asset_type_arr_name, mem_arena, asset_type_arr_name_terminated))
            {
                ZF_REPORT_ERROR();
                return false;
            }
            ...
            {
                LogError(
                    "Packing instructions JSON \"%\" array does not exist or it is of the wrong "
                    "type!",
                    asset_type_arr_name);
                return false;
            }
            ...
            cJSON_ArrayForEach(cj_asset, cj_assets)
            {
                ...
                const auto fields = [asset_type_index]() -> s_array_rdonly<s_asset_field>
                {
                    ...
                    {
                        case ek_asset_type_texture: return g_tex_fields;

                        case ek_asset_type_font: return g_font_fields;

                        case ek_asset_type_shader_prog: return g_shader_prog_fields;

                        case ek_asset_type_sound: return g_snd_fields;
                    }

                    return {};
                }();

                const auto field_vals = [asset_type_index, &tex_field_cj_ptrs, &font_field_cj_ptrs,
                                            &shader_prog_field_cj_ptrs,
                                            &snd_field_cj_ptrs]() -> s_array<cJSON*>
                {
                    switch (asset_type_index)
                    {
                        case ek_asset_type_texture: return tex_field_cj_ptrs;

                        case ek_asset_type_font: return font_field_cj_ptrs;

                        case ek_asset_type_shader_prog: return shader_prog_field_cj_ptrs;
                ...
                for (t_size fi = 0; fi < fields.len; fi++)
                {
                    ...
                    if (!field_vals[fi])
                    {
                        ...
                        LogError(
                            "A packing instructions JSON \"%\" entry is missing required field "
                            "\"%\"!",
                    ...
                    const auto is_valid = [fi, fields, field_vals]() -> t_b8
                    {
                        ...
                        {
                            case ek_asset_field_type_str: return cJSON_IsString(field_vals[fi]);

                            case ek_asset_field_type_num: return cJSON_IsNumber(field_vals[fi]);
                        }
                    ...
                    {
                        LogError(
                            "A packing instructions JSON \"%\" entry has field \"%\" as the wrong "
                            "type! Expected a %.",
                            asset_type_arr_name, field_name,
                ...
                switch (asset_type_index)
                {
                    ...
                    case ek_asset_type_font:
                    {
                        ...
                        const auto extra_chrs_fp =
                            StrFromRaw(field_vals[ek_font_field_extra_chrs_file_path]->valuestring);
                        ...
                        SetBitsInRange(code_pt_bvs[0], g_printable_ascii_range_begin,
                            g_printable_ascii_range_end); // Add the printable ASCII range as a
                                                          // default.
                        ...
                        if (field_vals[ek_font_field_extra_chrs_file_path])
                        {
                            ...
                            if (!LoadFileContents(
                                    extra_chrs_fp, mem_arena, mem_arena, extra_chrs_file_contents))
                            {
                                LogError(
                                ...
                                load_from_raw_res, &code_pt_bvs[1]))
                        {
                            switch (load_from_raw_res)
                            {
                                case ek_font_load_from_raw_result_unsupported_code_pt:
                                    ...
                                    ZF_FOR_EACH_SET_BIT(code_pt_bvs[1], i) { Log("- %", i); }
