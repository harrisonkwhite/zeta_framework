...
namespace zf {
    ...
    t_b8 PackAssets(const strs::StrRdonly instrs_json_file_path) {
        ...
        for (t_i32 asset_type_index = 0; asset_type_index < eks_asset_type_cnt; asset_type_index++) {
            ...
            cJSON_ArrayForEach(cj_asset, cj_assets) {
                ...
                case ek_asset_type_texture: {
                    ...
                    if (!gfx::load_texture_from_raw(file_path, &arena, &arena, &texture_data)) {
                        LogError(ZF_STR_LITERAL("Failed to load texture from file \"%\"!"), file_path);
                        return false;
                    }

                    if (!gfx::pack_texture(out_file_path, texture_data, &arena)) {
                ...
                case ek_asset_type_font: {
                    ...
                    const auto code_pt_bv = f_mem_push_item_zeroed<strs::CodePointBitVector>(&arena);

                    f_mem_set_bits_in_range(*code_pt_bv, strs::g_printable_ascii_range_begin, strs::g_printable_ascii_range_end); // Add the printable ASCII range as a default.
                    ...
                    if (!gfx::load_font_from_raw(file_path, height, code_pt_bv, &arena, &arena, &arena, &arrangement, &atlas_rgbas)) {
                        LogError(ZF_STR_LITERAL("Failed to load font from file \"%\"!"), file_path);
                        return false;
                    }

                    if (!gfx::pack_font(out_file_path, arrangement, atlas_rgbas, &arena)) {
                ...
                case ek_asset_type_shader: {
                    ...
                    if (!gfx::pack_shader(out_file_path, compiled_bin, &arena)) {
