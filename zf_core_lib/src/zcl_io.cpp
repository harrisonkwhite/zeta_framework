...
namespace zf
{
    t_b8 OpenFile(const s_str_rdonly file_path,
    ...
    t_b8 LoadFileContents(const s_str_rdonly file_path,
        ...
        e_directory_creation_result* const o_creation_res)
    {
        ...
        if (o_creation_res)
        {
            switch (errno)
            {
                case EEXIST: *o_creation_res = ek_directory_creation_result_already_exists; break;

                case EACCES:
                case EPERM: *o_creation_res = ek_directory_creation_result_permission_denied; break;

                case ENOENT: *o_creation_res = ek_directory_creation_result_path_not_found; break;

                default: *o_creation_res = ek_directory_creation_result_unknown_err; break;
            }
        }
    ...
    t_b8 CreateDirectoryAndParents(const s_str_rdonly path,
        s_mem_arena& temp_mem_arena,
        e_directory_creation_result* const o_dir_creation_res)
    {
        ...
        const auto create_dir_if_nonexistent = [&temp_mem_arena, o_dir_creation_res](
                                                   const s_str_rdonly path)
        {
    ...
    t_b8 CreateFileAndParentDirs(const s_str_rdonly path,
        s_mem_arena& temp_mem_arena,
        e_directory_creation_result* const o_dir_creation_res)
    {
        if (o_dir_creation_res)
