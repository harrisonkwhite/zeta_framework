...
namespace zf
{
    ...
    // @section: Bits
    // ============================================================
    static t_size IndexOfFirstSetBitHelper(
        const s_bit_vec_rdonly& bv, const t_size from, const t_u8 xor_mask)
    {
        ZF_ASSERT(IsBitVecValid(bv));
        ZF_ASSERT(from >= 0 && from <= bv.bit_cnt); // Intentionally allowing the upper bound here
                                                    // for the case of iteration.
