...
namespace zf {
    ...
    inline t_i32 gen_i32(t_rng *const rng) {
        return static_cast<t_i32>(f_rand_gen_u32(rng));
    }
    ...
    inline t_f32 f_rand_gen_f32_in_range(t_rng *const rng, const t_f32 min_incl, const t_f32 max_excl) {
        return min_incl + (f_rand_gen_perc(rng) * (max_excl - min_incl));
    }
}
