...
namespace zf {
    ...
    typename tp_list_type::t_elem *AppendDynamic(tp_list_type *const list, const typename tp_list_type::t_elem &val, s_arena *const extension_arena, const t_list_extension_cap_calculator extension_cap_calculator = g_default_list_extension_cap_calculator) {
    ...
    template <co_list tp_list_type>
    typename tp_list_type::t_elem *InsertDynamic(tp_list_type *const list, const t_i32 index, const typename tp_list_type::t_elem &val, s_arena *const extension_arena, const t_list_extension_cap_calculator extension_cap_calculator = g_default_list_extension_cap_calculator) {
        if (list->len == list->Cap()) {
            Extend(list, extension_arena, extension_cap_calculator);
        }
    ...
    template <co_list tp_list_type>
    void RemoveAtSwapback(tp_list_type *const list, const t_i32 index) {
        ZF_ASSERT(list->len > 0);
        ZF_ASSERT(index >= 0 && index < list->len);
