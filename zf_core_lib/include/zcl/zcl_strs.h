...
// The cost however is that interop with C standard library functions and most other libraries is
// more awkward, though these are insignificant portions of the code. The approach generally is to
// make a terminated clone of the string on a temporary memory arena. Since this is just an arena
// push as opposed to a heap allocation, the cost is trivial.
...
namespace zf
{
    ...
    [[nodiscard]] inline t_b8 CloneStrButAddTerminator(
        const s_str_rdonly src, s_mem_arena& mem_arena, s_str& o_clone)
    {
        o_clone = {};
    ...
    t_b8 WalkStr(const s_str_rdonly str,
        t_size& byte_index,
        s_str_chr_info& o_chr_info); // Returns false iff the walk has ended.
    t_b8 WalkStrReverse(const s_str_rdonly str,
        t_size& byte_index,
        s_str_chr_info&
            o_chr_info); // Returns false iff the walk has ended. For a full walk, the byte index
                         // has to be initialised to the index of ANY BYTE in the last character.

    t_unicode_code_pt UTF8ChrBytesToCodePoint(
        const s_array_rdonly<t_u8> bytes); // Only accepts bytes representing a unicode code point
                                           // in an array with a length in [1, 4].
    void MarkStrCodePoints(const s_str_rdonly str,
        t_unicode_code_pt_bit_vec& code_pts); // Sets the bits associated with each unicode code
                                              // point that appear in the input string.

#define ZF_WALK_STR(str, chr_info)                                                  \
    for (t_size ZF_CONCAT(p_bi_l, __LINE__) = 0; ZF_CONCAT(p_bi_l, __LINE__) != -1; \
        ZF_CONCAT(p_bi_l, __LINE__) = -1)                                           \
        for (s_str_chr_info chr_info; WalkStr(str, ZF_CONCAT(p_bi_l, __LINE__), chr_info);)

#define ZF_WALK_STR_REVERSE(str, chr_info)                                                         \
    for (t_size ZF_CONCAT(p_bi_l, __LINE__) = str.bytes.len - 1;                                   \
        ZF_CONCAT(p_bi_l, __LINE__) != str.bytes.len; ZF_CONCAT(p_bi_l, __LINE__) = str.bytes.len) \
