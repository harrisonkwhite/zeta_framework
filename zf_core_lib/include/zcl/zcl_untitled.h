...
namespace zf::gfx {
    ...
    // There should be an arena implicitly in here that frees up all resources.
    s_ptr<s_context> CreateContext(); // @todo: This should take in the glfwProcAddress thing.
    ...
    enum e_resource_type {
        ek_resource_type_mesh_static,
        ek_resource_type_mesh_dynamic, // Vertices change.
        ek_resource_type_shader_prog,  // The public interface should work such that this assumes ANY shader language.
    ...
    [[nodiscard]] t_b8 CreateMesh(const s_context &context, const s_ptr<const t_f32> verts_ptr, const t_len verts_len, const s_array_rdonly<t_u16> elems, const s_array_rdonly<t_i32> vert_attr_lens, s_ptr<s_resource> &o_mesh, s_ptr<s_resource_arena> arena = nullptr);
    [[nodiscard]] t_b8 CreateMesh(const s_context &context, const s_ptr<const t_f32> verts_ptr, const t_len verts_len, const s_array_rdonly<t_u16> elems, const s_array_rdonly<t_i32> vert_attr_lens, s_ptr<s_resource> &o_mesh, s_ptr<s_resource_arena> arena = nullptr);

    [[nodiscard]] t_b8 CreateTexture(const s_context &context, const s_texture_data_rdonly tex_data, s_ptr<s_resource> &o_tex, s_ptr<s_resource_arena> arena = nullptr);

    [[nodiscard]] t_b8 CreateShaderProgram(const s_context &context, const s_ptr<t_u8> vert_shader_blob, const s_ptr<t_u8> frag_shader_blob, s_ptr<s_resource> &o_prog, s_ptr<s_resource_arena> arena = nullptr);

    // ============================================================
    // @section: Rendering
    // ============================================================

    void SetShaderProgramUniform(const, const s_str name, something val); // @todo: What happens to uniforms after a draw occurs with a shader program? Do they just keep the old state? Does each shader program have its uniforms only once?

    void SetViewport(const s_context &context, const s_v2_i size);
    void Clear(const s_context &context, const s_color_rgb24f col);
    void RenderMesh(const s_context &context, const s_resource &mesh, const s_resource &shader_prog, const s_ptr<const s_resource> tex = nullptr);
}

namespace zf {
    struct s_texture_asset {
        s_ptr<gfx::s_resource> hdl;
    };

    struct s_font_asset {
        s_array<s_ptr<gfx::s_resource>> atlas_texture_hdls;
    };
}
#endif
