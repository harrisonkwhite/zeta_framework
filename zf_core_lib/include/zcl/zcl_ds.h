...
namespace zf
{
    ...
    template <typename tp_type>
    void ListInsert(
        const s_array<tp_type> backing_arr, t_size& len, const t_size index, const tp_type& val)
    {
        ZF_ASSERT(len >= 0 && len < backing_arr.len);
    ...
    template <typename tp_type>
    [[nodiscard]] t_b8 MakeList(
        s_mem_arena& mem_arena, const t_size cap, s_list<tp_type>& o_list, const t_size len = 0)
    {
    ...
    template <typename tp_type>
    t_b8 MakeStack(s_mem_arena& mem_arena,
        const t_size cap,
        s_stack<tp_type>& o_stack,
    ...
    template <typename tp_type>
    tp_type& Enqueue(const s_array<tp_type> backing_arr,
        const t_size begin_index,
        t_size& len,
        const tp_type& val)
    {
        ZF_ASSERT(len >= 0 && len < backing_arr.len);
    ...
    template <typename tp_type>
    [[nodiscard]] t_b8 MakeQueue(
        s_mem_arena& mem_arena, const t_size cap, s_queue<tp_type>& o_queue, const t_size len = 0)
    {
        ZF_ASSERT(cap > 0 && len >= 0 && len <= cap);
    ...
    [[nodiscard]] t_b8 InitHashMapBackingBlock(
        s_hash_map_backing_block<tp_key_type, tp_val_type>* bb,
        const t_size cap,
    ...
    [[nodiscard]] t_b8 HashMapBackingBlockGet(
        const s_hash_map_backing_block<tp_key_type, tp_val_type>* bb,
    ...
    [[nodiscard]] e_hash_map_put_result HashMapBackingBlockPut(
        ...
        t_size& index,
        const t_size new_bb_cap,
        s_mem_arena& new_bb_mem_arena)
    {
        ZF_ASSERT(index >= -1);
    ...
    // they're needed.
    template <typename tp_key_type, typename tp_val_type>
    [[nodiscard]] t_b8 InitHashMap(s_hash_map<tp_key_type, tp_val_type>* const hm,
    ...
    template <typename tp_type>
    t_size KeyToHashIndex(
        const tp_type& key, const t_hash_func<tp_type> hash_func, const t_size table_size)
    {
        ZF_ASSERT(hash_func);
    ...
    template <typename tp_key_type, typename tp_val_type>
    [[nodiscard]] e_hash_map_put_result HashMapPut(
        s_hash_map<tp_key_type, tp_val_type>& hm, const tp_key_type& key, const tp_val_type& val)
    {
        const t_size hash_index = KeyToHashIndex(key, hm.hash_func, hm.immediate_indexes.len);

        const auto res = HashMapBackingBlockPut(hm.backing_blocks_head, key, hm.key_comparator, val,
            hm.immediate_indexes[hash_index], hm.backing_block_cap, *hm.mem_arena);
    ...
    t_b8 HashMapRemove(s_hash_map<tp_key_type, tp_val_type>& hm, const tp_key_type key)
    {
        ...
        if (HashMapBackingBlockRemove(
                hm.backing_blocks_head, key, hm.immediate_indexes[hash_index]))
        {
            hm.kv_pair_cnt--;
            return true;
        }
    ...
    // memory arena!
    template <typename tp_key_type, typename tp_val_type>
    [[nodiscard]] t_b8 SerializeHashMap(
        s_stream& stream, const s_hash_map<tp_key_type, tp_val_type>& hm)
    {
        ...
        while (bb)
        {
            if (!SerializeArray(stream, bb->keys) && !SerializeArray(stream, bb->vals) &&
                !SerializeArray(stream, bb->next_indexes) && !SerializeBitVec(stream, bb->usage))
            {
                return false;
            }
    ...
    template <typename tp_key_type, typename tp_val_type>
    [[nodiscard]] t_b8 DeserializeHashMap(s_mem_arena& mem_arena,
        s_stream& stream,
        const t_hash_func<tp_key_type> hash_func,
        const t_bin_comparator<tp_key_type> key_comparator,
