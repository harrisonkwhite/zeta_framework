...
namespace zf {
    ...
    struct s_static_list {
        ...
        tp_type &operator[](const t_len index) {
            ZF_ASSERT(index < len);
            return backing_arr[index];
        }

        const tp_type &operator[](const t_len index) const {
            ZF_ASSERT(index < len);
            return backing_arr[index];
        }
    };
    ...
    template <typename tp_type>
    tp_type &ListAppend(const s_array<tp_type> backing_arr, t_len &len, const tp_type &val) {
    ...
    template <typename tp_type>
    void ListInsert(const s_array<tp_type> backing_arr, t_len &len, const t_len index, const tp_type &val) {
        ZF_ASSERT(len >= 0 && len < backing_arr.len);
        ZF_ASSERT(index >= 0 && index <= len);

        for (t_len i = len; i > index; i--) {
    ...
    template <typename tp_type>
    void ListInsert(s_list<tp_type> &list, const t_len index, const tp_type &val) {
        ListInsert(list.backing_arr, list.len, index, val);
    }

    template <typename tp_type, t_len tp_cap>
    void ListInsert(s_static_list<tp_type, tp_cap> &list, const t_len index, const tp_type &val) {
        ListInsert(ToNonstaticArray(list.backing_arr), list.len, index, val);
    }

    template <typename tp_type>
    tp_type ListRemoveLast(const s_array<tp_type> backing_arr, t_len &len) {
        ZF_ASSERT(len > 0 && len <= backing_arr.len);
    ...
    template <typename tp_type>
    void ListRemoveSwapback(s_list<tp_type> &list, const t_len index) {
        ListRemoveSwapback(list.backing_arr, list.len, index);
    }

    template <typename tp_type, t_len tp_cap>
    void ListRemoveSwapback(s_static_list<tp_type, tp_cap> &list, const t_len index) {
        ListRemoveSwapback(ToNonstaticArray(list.backing_arr), list.len, index);
    }

    template <typename tp_type>
    void ListRemove(const s_array<tp_type> backing_arr, t_len &len, const t_len index) {
        ZF_ASSERT(len > 0 && len <= backing_arr.len);
        ZF_ASSERT(index >= 0 && index < len);
    ...
    template <typename tp_type>
    void ListRemove(s_list<tp_type> &list, const t_len index) {
        ListRemove(list.backing_arr, list.len, index);
    }

    template <typename tp_type, t_len tp_cap>
    void ListRemove(s_static_list<tp_type, tp_cap> &list, const t_len index) {
        ListRemove(ToNonstaticArray(list.backing_arr), list.len, index);
    }

    template <typename tp_type>
    [[nodiscard]] t_b8 MakeList(s_mem_arena &mem_arena, const t_len cap, s_list<tp_type> &o_list, const t_len len = 0) {
        ZF_ASSERT(cap > 0 && len >= 0 && len <= cap);

        o_list = {.len = len};

        return AllocArray(mem_arena, cap, o_list.backing_arr);
    }
    ...
    template <typename tp_type>
    tp_type &StackTop(const s_array<tp_type> backing_arr, const t_len height) {
    ...
    template <typename tp_type>
    tp_type &StackPush(const s_array<tp_type> backing_arr, t_len &height, const tp_type &val) {
        ZF_ASSERT(height >= 0 && height < backing_arr.len);
    ...
    template <typename tp_type>
    tp_type StackPop(const s_array<tp_type> backing_arr, t_len &height) {
        ZF_ASSERT(height > 0 && height <= backing_arr.len);
    ...
    template <typename tp_type>
    t_b8 MakeStack(s_mem_arena &mem_arena, const t_len cap, s_stack<tp_type> &o_stack, const t_len height = 0) {
        ZF_ASSERT(cap > 0 && height >= 0 && height <= cap);

        o_stack = {.height = height};

        return AllocArray(mem_arena, cap, o_stack.backing_arr);
    }
    ...
    template <typename tp_type>
    tp_type &Enqueue(const s_array<tp_type> backing_arr, const t_len begin_index, t_len &len, const tp_type &val) {
        ZF_ASSERT(len >= 0 && len < backing_arr.len);
    ...
    template <typename tp_type>
    tp_type Dequeue(const s_array_rdonly<tp_type> backing_arr, t_len &begin_index, t_len &len) {
        ZF_ASSERT(len > 0 && len <= backing_arr.len);
    ...
    template <typename tp_type>
    [[nodiscard]] t_b8 MakeQueue(s_mem_arena &mem_arena, const t_len cap, s_queue<tp_type> &o_queue, const t_len len = 0) {
        ZF_ASSERT(cap > 0 && len >= 0 && len <= cap);

        o_queue = {.len = len};

        return AllocArray(mem_arena, cap, o_queue.backing_arr);
    }
    ...
    template <typename tp_key_type, typename tp_val_type>
    [[nodiscard]] t_b8 InitHashMapBackingBlock(s_hash_map_backing_block<tp_key_type, tp_val_type> *bb, const t_len cap, s_mem_arena *const mem_arena) {
        ZF_ASSERT(cap >= 0);
        ...
        if (!AllocArray(&bb->keys, cap, mem_arena)) {
            return false;
        }

        if (!AllocArray(&bb->vals, cap, mem_arena)) {
            return false;
        }

        if (!AllocArray(&bb->next_indexes, cap, mem_arena)) {
            return false;
        }

        if (!CreateBitVec(&bb->usage, cap, mem_arena)) {
            return false;
        }
    ...
    template <typename tp_key_type, typename tp_val_type>
    t_len HashMapBackingBlockCap(const s_hash_map_backing_block<tp_key_type, tp_val_type> *const bb) {
        return bb->keys.len;
    }

    template <typename tp_key_type, typename tp_val_type>
    [[nodiscard]] t_b8 HashMapBackingBlockGet(const s_hash_map_backing_block<tp_key_type, tp_val_type> *bb, const tp_key_type key, const t_bin_comparator<tp_key_type> key_comparator, t_len index, tp_val_type *const o_val) {
        if (!bb || index == -1) {
            return false;
        }
    ...
    template <typename tp_key_type, typename tp_val_type>
    [[nodiscard]] e_hash_map_put_result HashMapBackingBlockPut(s_hash_map_backing_block<tp_key_type, tp_val_type> *bb, const tp_key_type key, const t_bin_comparator<tp_key_type> key_comparator, const tp_val_type val, t_len &index, const t_len new_bb_cap, s_mem_arena &new_bb_mem_arena) {
        ZF_ASSERT(index >= -1);
    ...
    template <typename tp_key_type, typename tp_val_type>
    t_b8 HashMapBackingBlockRemove(s_hash_map_backing_block<tp_key_type, tp_val_type> *bb, const tp_key_type key, const t_bin_comparator<tp_key_type> key_comparator, t_len &index) {
        ZF_ASSERT(index >= -1);
    ...
    // they're needed.
    template <typename tp_key_type, typename tp_val_type>
    [[nodiscard]] t_b8 CreateHashMap(const t_hash_func<tp_key_type> hash_func, s_mem_arena *const mem_arena, s_hash_map<tp_key_type, tp_val_type> *const o_hm, const t_len immediate_cap = g_hash_map_immediate_cap_default, const t_len backing_block_cap = g_hash_map_backing_block_cap_default, const t_bin_comparator<tp_key_type> key_comparator = DefaultBinComparator) {
        MarkUninitted(o_hm);

        o_hm->hash_func = hash_func;
        o_hm->key_comparator = key_comparator;
        o_hm->kv_pair_cnt = 0;
        o_hm->backing_block_cap = backing_block_cap;
        o_hm->mem_arena = mem_arena;
        o_hm->backing_blocks_head = nullptr;

        if (!AllocArray(immediate_cap, mem_arena, &o_hm->immediate_indexes)) {
            return false;
        }

        SetAllTo(o_hm->immediate_indexes, -1);
    ...
    // memory arena!
    template <typename tp_key_type, typename tp_val_type>
    [[nodiscard]] t_b8 SerializeHashMap(s_stream *const stream, const s_hash_map<tp_key_type, tp_val_type> &hm) {
        if (!StreamWriteItem(stream, hm.kv_pair_cnt) && !SerializeArray(stream, hm.immediate_indexes) && !StreamWriteItem(stream, hm.backing_block_cap)) {
            return false;
        }
        ...
        while (bb) {
            if (!SerializeArray(stream, bb->keys) && !SerializeArray(stream, bb->vals) && !SerializeArray(stream, bb->next_indexes) && !SerializeBitVec(stream, bb->usage)) {
                return false;
            }
    ...
    [[nodiscard]] t_b8 DeserializeHashMap(s_stream *const stream, s_mem_arena *const hm_mem_arena, const t_hash_func<tp_key_type> hm_hash_func, const t_bin_comparator<tp_key_type> hm_key_comparator, s_hash_map<tp_key_type, tp_val_type> *const o_hm) {
        ...
        for (t_len i = 0; i < bb_cnt; i++) {
            const auto bb = PushToMemArena<s_hash_map_backing_block<tp_key_type, tp_val_type>>(&hm_mem_arena);
            ...
            *bb_ptr_to_update = bb;
            ...
            if (!DeserializeArray(stream, hm_mem_arena, bb->keys) && !DeserializeArray(stream, hm_mem_arena, bb->vals) && !DeserializeArray(stream, hm_mem_arena, bb->next_indexes) && !DeserializeBitVec(stream, hm_mem_arena, bb->usage)) {
                return false;
            }
