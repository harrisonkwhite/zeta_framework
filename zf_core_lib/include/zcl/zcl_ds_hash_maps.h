...
namespace zf {
    ...
    s_kv_store_block<tp_key_type, tp_val_type> *CreateKVStoreBlock(const t_i32 cap, t_arena *const arena) {
        const auto block = f_mem_push_item<s_kv_store_block<tp_key_type, tp_val_type>>(arena);

        block->keys = f_mem_push_array<tp_key_type>(arena, cap);
    ...
    template <c_simple tp_key_type, c_simple tp_val_type>
    [[nodiscard]] t_b8 FindInChain(const s_kv_store<tp_key_type, tp_val_type> *const kv_store, const t_i32 chain_begin_index, const tp_key_type &key, tp_val_type **const o_val) {
    ...
    e_kv_store_put_result PutInChain(s_kv_store<tp_key_type, tp_val_type> *const kv_store, t_i32 *const chain_begin_index, const tp_key_type &key, const tp_val_type &val) {
        ...
        *index = [kv_store, &key, &val]() {
            ...
            while (block) {
                const auto possible_rel_index_to_use = f_mem_index_of_first_unset_bit(block->usage);
    ...
    template <c_simple tp_key_type, c_simple tp_val_type>
    t_b8 RemoveInChain(s_kv_store<tp_key_type, tp_val_type> *const kv_store, t_i32 *const chain_begin_index, const tp_key_type &key) {
        ZF_ASSERT(*chain_begin_index >= -1 && *chain_begin_index < kv_store->block_cap * kv_store->block_cnt);
    ...
    template <c_simple tp_key_type, c_simple tp_val_type>
    t_i32 LoadChain(const s_kv_store<tp_key_type, tp_val_type> *const kv_store, const t_i32 begin_index, const t_array_mut<tp_key_type> keys, const t_array_mut<tp_val_type> vals) {
    ...
    template <c_simple tp_key_type, c_simple tp_val_type>
    [[nodiscard]] t_b8 HashMapPut(const s_hash_map<tp_key_type, tp_val_type> *const hash_map, const tp_key_type &key, tp_val_type **const o_val) {
    ...
    template <c_simple tp_key_type, c_simple tp_val_type>
    t_b8 HashMapRemove(s_hash_map<tp_key_type, tp_val_type> *const hash_map, const tp_key_type &key) {
        const t_i32 hash_index = KeyToHashIndex(key, hash_map->hash_func, HashMapCap(hash_map));
    ...
    template <c_simple tp_key_type, c_simple tp_val_type>
    void HashMapLoadEntries(const s_hash_map<tp_key_type, tp_val_type> *const hash_map, const t_array_mut<tp_key_type> keys, const t_array_mut<tp_val_type> vals) {
        ...
        for (t_i32 i = 0; i < hash_map->immediate_indexes.len; i++) {
            loaded_cnt += LoadChain(&hash_map->kv_store, hash_map->immediate_indexes[i], f_mem_slice_array_from(keys, loaded_cnt), f_mem_slice_array_from(vals, loaded_cnt));
        }
    }
    ...
    void HashMapLoadEntries(const s_hash_map<tp_key_type, tp_val_type> *const hash_map, t_arena *const arena, t_array_mut<tp_key_type> *const o_keys, t_array_mut<tp_val_type> *const o_vals) {
        *o_keys = f_mem_push_array<tp_key_type>(arena, HashMapEntryCount(hash_map));
        *o_vals = f_mem_push_array<tp_val_type>(arena, HashMapEntryCount(hash_map));
        return HashMapLoadEntries(hash_map, *o_keys, *o_vals);
    }

    template <c_simple tp_key_type, c_simple tp_val_type>
    [[nodiscard]] t_b8 SerializeHashMap(s_stream *const stream, const s_hash_map<tp_key_type, tp_val_type> *const hm, t_arena *const temp_arena) {
    ...
    template <c_simple tp_key_type, c_simple tp_val_type>
    [[nodiscard]] t_b8 DeserializeHashMap(s_stream *const stream, t_arena *const hm_arena, const t_hash_func<tp_key_type> hm_hash_func, t_arena *const temp_arena, s_hash_map<tp_key_type, tp_val_type> *const o_hm, const t_comparator_bin<tp_key_type> hm_key_comparator = g_comparator_bin_default<tp_key_type>) {
        t_i32 cap;
        ...
        *o_hm = HashMapCreate<tp_key_type, tp_val_type>(hm_hash_func, hm_arena, cap, hm_key_comparator);
        ...
        const auto keys = f_mem_push_array<tp_key_type>(temp_arena, entry_cnt);

        if (!ReadItemsIntoArray(stream, keys, entry_cnt)) {
            return false;
        }

        const auto vals = f_mem_push_array<tp_val_type>(temp_arena, entry_cnt);
