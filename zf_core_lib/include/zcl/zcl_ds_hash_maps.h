...
namespace zf {
    ...
    inline const t_hash_func<s_str_rdonly> g_str_hash_func =
        [](const s_str_rdonly &key) {
    ...
    template <co_simple tp_key_type, co_simple tp_val_type>
    s_hash_map<tp_key_type, tp_val_type> CreateHashMap(const t_hash_func<tp_key_type> hash_func, s_arena *const arena, const t_i32 cap = g_hash_map_cap_default, const t_comparator_bin<tp_key_type> key_comparator = g_comparator_bin_default<tp_key_type>) {
    ...
    template <co_simple tp_key_type, co_simple tp_val_type>
    t_i32 Cap(const s_hash_map<tp_key_type, tp_val_type> *const hash_map) {
        return hash_map->immediate_indexes.len;
    }

    template <co_simple tp_key_type, co_simple tp_val_type>
    t_i32 EntryCount(const s_hash_map<tp_key_type, tp_val_type> *const hash_map) {
        return hash_map->kv_store.pair_cnt;
    }

    template <co_simple tp_key_type, co_simple tp_val_type>
    [[nodiscard]] t_b8 Find(const s_hash_map<tp_key_type, tp_val_type> *const hash_map, const tp_key_type &key, tp_val_type **const o_val) {
        const t_i32 hash_index = KeyToHashIndex(key, hash_map->hash_func, Cap(hash_map));
    ...
    template <co_simple tp_key_type, co_simple tp_val_type>
    e_hash_map_put_result Put(s_hash_map<tp_key_type, tp_val_type> *const hash_map, const tp_key_type &key, const tp_val_type &val) {
        const t_i32 hash_index = KeyToHashIndex(key, hash_map->hash_func, Cap(hash_map));
    ...
    template <co_simple tp_key_type, co_simple tp_val_type>
    t_b8 Remove(s_hash_map<tp_key_type, tp_val_type> *const hash_map, const tp_key_type &key) {
        const t_i32 hash_index = KeyToHashIndex(key, hash_map->hash_func, Cap(hash_map));
    ...
    template <co_simple tp_key_type, co_simple tp_val_type>
    void LoadEntries(const s_hash_map<tp_key_type, tp_val_type> *const hash_map, const s_array_mut<tp_key_type> keys, const s_array_mut<tp_val_type> vals) {
        ZF_ASSERT(keys.len >= EntryCount(hash_map) && vals.len >= EntryCount(hash_map));

        t_i32 loaded_cnt = 0;

        for (t_i32 i = 0; i < hash_map->immediate_indexes.len; i++) {
            loaded_cnt += LoadChain(&hash_map->kv_store, hash_map->immediate_indexes[i], SliceFrom(keys, loaded_cnt), SliceFrom(vals, loaded_cnt));
        }
    }

    // Allocates the given arrays with the memory arena and loads key-value pairs into them.
    template <co_simple tp_key_type, co_simple tp_val_type>
    void LoadEntries(const s_hash_map<tp_key_type, tp_val_type> *const hash_map, s_arena *const arena, s_array_mut<tp_key_type> *const o_keys, s_array_mut<tp_val_type> *const o_vals) {
        *o_keys = PushArray<tp_key_type>(arena, EntryCount(hash_map));
        *o_vals = PushArray<tp_val_type>(arena, EntryCount(hash_map));
        return LoadEntries(hash_map, *o_keys, *o_vals);
    }
