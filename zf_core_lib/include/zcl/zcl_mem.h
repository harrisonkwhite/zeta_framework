...
namespace zf {
    ...
    s_array_mut<tp_type> AllocArray(const t_i32 len, s_arena *const arena) {
        ...
        return {static_cast<tp_type *>(PushToArena(arena, ZF_SIZE_OF(tp_type) * len, ZF_ALIGN_OF(tp_type))), len};
    }
    ...
    struct s_bit_vec_rdonly {
        ...
        constexpr s_bit_vec_rdonly(const s_array_rdonly<t_u8> bytes, const t_i32 bit_cnt) : bytes(bytes), bit_cnt(bit_cnt) {}
    ...
    struct s_bit_vec_mut {
        ...
        constexpr s_bit_vec_mut(const s_array_mut<t_u8> bytes, const t_i32 bit_cnt) : bytes(bytes), bit_cnt(bit_cnt) {}
        constexpr s_bit_vec_mut(const s_array_mut<t_u8> bytes) : bytes(bytes), bit_cnt(BytesToBits(bytes.len)) {}
    ...
    constexpr void ApplyMaskToBits(const s_bit_vec_mut dest, const s_bit_vec_rdonly src, const e_bitwise_mask_op op) {
        ZF_ASSERT(IsValid(dest));
        ZF_ASSERT(IsValid(src));
        ...
        dest.bytes[dest.bytes.len - 1] &= dest.LastByteMask();
    }
