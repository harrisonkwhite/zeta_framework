...
namespace zf
{
    ...
    // Gives the digit at the given index, where the indexes are from the least significant digit to
    // the most.
    template <c_integral tp_type>
    ...
    struct s_rect
    {
        ...
        constexpr s_rect(
            const tp_type x, const tp_type y, const tp_type width, const tp_type height)
    ...
    // rectangle.
    template <c_numeric tp_type>
    s_rect<tp_type> ClampedSubrect(
        const s_rect<tp_type> inner_rect, const s_rect<tp_type> outer_rect)
    {
    ...
    // Generate a rectangle encompassing all of the provided rectangles. At least a single rectangle
    // must be provided.
    template <c_numeric tp_type>
    ...
    inline s_matrix_4x4 MakeOrthographicMatrix4x4(const t_f32 left,
        const t_f32 right,
        const t_f32 bottom,
        const t_f32 top,
        const t_f32 z_near,
